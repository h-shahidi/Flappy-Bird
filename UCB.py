...
def trainNetwork(model,args):
    ...
    while (True):
        ...
        printInfo(t, state, epsilon, action_index, r_t, Q_sa, Q_sa+np.sqrt(2*np.log(t)/(Nt)), loss)


    print("Episode finished!")
    print("************************")

def printInfo(t, state, epsilon, action_index, r_t, Q_sa, modified_Q_sa, loss):
    print("TIMESTEP", t, "/ STATE", state, \
          "/ EPSILON", epsilon, "/ ACTION", action_index, "/ REWARD", r_t, \
          "/ Q_MAX " , np.max(Q_sa), "/ MODIFIED ", np.max(modified_Q_sa), \
          "/ Loss ", loss)
